...
AI_CONFIG_FILE = os.path.join(BASE_DIR, 'AI設定.txt')
...
def load_ai_api_key():
    """AI設定.txtからHugging Faceトークンを読み込む。"""
    try:
...
def create_huggingface_client():
    """Hugging Face用のOpenAI互換クライアントを作成する。"""
...
def test_huggingface_connection(client):
    """Hugging Face APIへの接続を実際に確認する。"""
...
def timeout_youtube_user(youtube, live_chat_id, channel_id, duration_seconds=300):
    """YouTubeライブチャットでユーザーを一時タイムアウトする。"""
    ...
    return youtube.liveChatBans().insert(
        part='snippet',
        body=body
    ).execute()
...
def get_youtube():
    ...
    # token.json がない、または認証情報が無効なら初回認証
...
def get_live_info(youtube, video_id):
    ...
    if not chat_id:
        raise ValueError(
            '現在取得可能なライブチャットがありません。'
        )

    return (
        item.get('snippet', {}).get('title', '不明'),
        chat_id
    )
...
def check_repeat_comment(
    ...
    for item in history:
        ...
        if (
            item.get('comment') == comment_text
            and now - item.get('time', 0) <= 10
        ):
            recent_count += 1
...
def first_stage_check(text, account_age_hours, is_first_observed):
    ...
    ai_related = [
        s for s in signals
        if s != 'new_account_first_comment'
    ]

    return {
        'signals': signals,
        'need_ai': bool(ai_related)
    }
...
def calculate_ai_score(ai):
    ...
    if ai.get('advertising'):
        s += 3

    if ai.get('context'):
        s += 3

    if ai.get('similarity'):
        s += 3

    if ai.get('pigeon'):
        s += 3

    elif ai.get('semi_pigeon'):
        s += 2
...
def analyze_comment_with_ai(
    comment_text,
    past_problem_comments=None,
    openai_client=None
):
    """Hugging Faceでコメントを分析する。"""
    ...
    try:
        ...
        if past_problem_comments:
            past_text = '\n'.join(
                str(x) for x in past_problem_comments[-5:]
            )

        prompt = f"""
あなたはYouTubeライブのコメント監視を補助するAIです。
...
過去の問題コメント:
{past_text if past_text else 'なし'}

次の6項目をtrue/falseで判定してください。

advertising:
商品、サービス、チャンネル、SNSなどへの宣伝・広告・登録誘導があるか

context:
このライブの文脈から見て迷惑行為・荒らし目的の可能性があるか

similarity:
過去の問題コメントと内容が似ているか

semi_pigeon:
別の配信や別のライブの話題を出しているが、
直接的な視聴誘導までは確認できないか

pigeon:
別の配信・ライブへ行くことを直接促しているか
...
"""
        ...
        response = openai_client.chat.completions.create(
            ...
            messages=[
                {
                    'role': 'system',
                    'content': 'あなたはYouTubeライブのコメント監視AIです。'
                },
                {
                    'role': 'user',
                    'content': prompt
        ...
        return {
            'available': True,
            'advertising': bool(
                data.get('advertising', False)
            ),
            'context': bool(
                data.get('context', False)
            ),
            'similarity': bool(
                data.get('similarity', False)
            ),
            'semi_pigeon': bool(
                data.get('semi_pigeon', False)
            ),
            'pigeon': bool(
                data.get('pigeon', False)
            ),
            'reason': str(
                data.get(
                    'reason',
        ...
        return {
            ...
            'semi_pigeon': False,
            'pigeon': False,
            'reason': f'AI接続エラー: {type(e).__name__}'
        }


def run_two_stage_judgement(
    comment_text,
    account_age_hours,
    is_first_observed,
    ai_enabled,
...
class LiveGuarderApp:
    ...
    def initialize_ai(self):
        """AI設定.txtを読み込み、Hugging Faceへの接続を確認する。"""

        self.openai_client = create_huggingface_client()
        ...
        self.ai_connected = test_huggingface_connection(
            self.openai_client
        )
    ...
    def build_ui(self):
        ...
        self.left_panel.columnconfigure(
            0,
            weight=1
        )
        ...
        self.warning_text.configure(
            yscrollcommand=warning_scroll.set
        )
    ...
    def toggle_left_only(self):
        ...
        if not compact:
            ...
            self.root.geometry(
                f"{compact_w}x{compact_h}"
            )

            self.layout_toggle_top.config(
                text="▶ 全画面に戻す"
            )

            self.layout_toggle_btn.config(
                text="▶ 全画面に戻す"
            )
    ...
    def append_warning_at_top(self, text):
        """Insert latest warning at the top."""
    ...
    def connect_youtube(self):
        ...
        try:
            ...
            name = items[0]['snippet'].get(
                'title',
                '不明'
            )

            self.connection_status.config(
                text=f'🟢 YouTube：接続済み（{name}）'
            )
    ...
    def check_live(self):
        ...
        try:
            ...
            title, chat = get_live_info(
                self.youtube,
                vid
            )

            self.live_chat_id = chat

            self.live_status.config(
                text=f'🟢 ライブ：確認済み | {title}'
            )
    ...
    def process_comment(self, item):
        ...
        # 連投チェック
        is_repeat = check_repeat_comment(
            self.user_history,
            cid,
            comment
        )

        result = run_two_stage_judgement(
            comment,
            age,
            first,
            self.ai_enabled,
        ...
        # 同じコメントを10秒以内に3件以上投稿していた場合、＋3点
        if is_repeat:
            result['total_score'] += 3
            result['judgment'] = judge_score(
                result['total_score']
            )
        self.user_history.setdefault(
            cid,
            []
        ).append({
            'comment': comment,
        ...
        # この配信中のユーザー別・累積得点を更新
        current_score = self.user_scores.get(cid, 0)
        current_score += result['total_score']
        self.user_scores[cid] = current_score

        # コメント単体ではなく、配信中の累積得点で判定する
        cumulative_result = dict(result)
        cumulative_result['total_score'] = current_score
        cumulative_result['judgment'] = judge_score(current_score)

        # 3～5点に初めて到達したユーザーを注意報として記録
        if (
            3 <= current_score < WARNING_THRESHOLD
            and cid not in self.alerted_users
        ):
            self.alerted_users.add(cid)
            self.stats['alerts'] += 1

            self.enqueue_warning(
                f'🟡 {username}\n'
                f'累積スコア：{current_score}点\n'
                f'荒らし行為を行っている可能性があります。ご注意ください',
                'warning'
            )

        # 6点以上に初めて到達したユーザーを警告・タイムアウト処理
        if (
            current_score >= WARNING_THRESHOLD
            and cid not in self.processed_users
        ):

            self.processed_users.add(cid)

            self.enqueue_warning(
                f'⚠️ {username}\n'
                f'累積スコア：{current_score}点\n'
                f'{username}は荒らしを行っている恐れがあります。タイムアウト等を推奨します',
                'danger'
            )
            ...
            if self.mode == 'WARNING':
                ...
                if ENABLE_YOUTUBE_ACTION:
                    ...
                    try:
                        ...
                        self.enqueue_warning(
                            f'⏱️ 荒らし行為が確認されたため、{username}をタイムアウトしました\n'
                            f'累積スコア：{current_score}点',
                            'danger'
                        )
        ...
        # コメント欄には今回の加点結果と累積得点を表示
        display_result = dict(result)
        display_result['total_score'] = current_score
        display_result['judgment'] = judge_score(current_score)
    ...
    def flush_ui(self):
        ...
        while self.comment_buffer:
            ...
            u, c, age, s, j, a, ai_result = (
                self.comment_buffer.pop(0)
            )
            ...
            text = (
                f'👤 {u}\n'
                f'📅 アカウント作成から：約{account_age_text}\n'
                f'💬 {c}\n'
                f'📊 {s}点 / {j} / {ai}\n'
            )

            # AIを実際に使用した場合だけ、判定理由を表示
            if a and ai_result:
        ...
        while self.warning_buffer:
            ...
            text, level = (
                self.warning_buffer.pop(0)
            )

            self.append_warning_at_top(
                text
            )
    ...
    def stats_text(self):
        ...
        return (
            f"💬 コメント：{self.stats['comments']}\n"
            f"⚠️ 注意報：{self.stats['alerts']}\n"
            f"🟡 警告：{self.stats['warnings']}\n"
            f"🔴 タイムアウト：{self.stats['timeouts']}"
        )
